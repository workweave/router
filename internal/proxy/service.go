...
func (s *Service) ProxyMessages(ctx context.Context, body []byte, w http.ResponseWriter, r *http.Request) error {
	...
	if routeRes.UsageBypass {
		err := s.bypassToAnthropic(ctx, env, feats, routeRes.modelSwitched(), string(routeRes.TurnType), time.Since(routeStart).Milliseconds(), requestStart, requestID, externalID, r, w)
