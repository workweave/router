...
// CredentialKey identifies a subscription credential without revealing it: a
// salted SHA-256 prefix of the token bytes. Safe to log/metric.
type CredentialKey string

// KeyFor derives the CredentialKey for a subscription token under salt. The salt
// is process-scoped (generated by the composition root) so a logged key can't be
