...
// ceiling for training content sent to the sidecar under the training_allowed gate.
const maxFeedbackDeltaChars = 256 * 1024

// clipFeedbackTrainingDelta caps an already-selected feedback exchange with a
// per-field + total budget. Newest message first; within a message, Text then
// input_json then tool_result.Text share the total budget.
