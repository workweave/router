...
**Single-binding same-binding retry.** Most catalog models carry one binding (Anthropic/OpenAI/Google), so cross-binding failover has nowhere to walk — a sole-provider 5xx/timeout would kill the turn. For these, `dispatchWithFallback` retries the *same* binding in place up to `maxSameBindingRetries` (2) with jittered exponential backoff (`sameBindingBackoff`: 250ms, 500ms base, +/-10% jitter so requests hitting the same blip don't retry in lockstep), pre-commit only, abortable on ctx cancel (`sleepWithContext`). Multi-binding models skip in-place retry (`len(bindings) > 1` breaks the inner loop) and fail straight over to the next provider — a different upstream beats re-hitting the flaky one. Tests inject `Service.retrySleep` to keep the backoff instant.

**Per-binding circuit breaker.** [`breaker.go`](breaker.go) holds a `sony/gobreaker/v2` `TwoStepCircuitBreaker` per `(provider, model)` binding (`breakerRegistry`, keyed via `breakerKey`). `dispatchWithFallback` calls `s.breakers.allow(key)` at the top of each binding iteration: if the breaker is open AND another binding remains, the binding is skipped without burning the retry/backoff budget; on the last (or only) binding it fails open (a possibly-recovered upstream beats a guaranteed 502) and records no outcome. After each binding's attempts, `recordOutcome(success)` reports to the breaker — `success = attemptErr == nil || !providers.IsRetryable(attemptErr)`, so only genuine transient/upstream failures count against it (client 4xx and post-commit stream errors do not). Trip after 5 consecutive failures, 30s open timeout, 2 half-open probes (all constants, no config surface). State is learned passively from live traffic — there is no health-probe loop. The registry is nil-safe: a bare `&Service{}` (most unit tests) has `breakers == nil`, so `allow` always permits and records nothing, preserving pre-breaker behavior; `NewService` wires a real registry.
