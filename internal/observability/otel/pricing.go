// Package otel provides the OTel span emitter and model catalog.
//
...
func AllPricing() map[string]Pricing {
	return catalog.AllPrimaryPricing()
}
...
// dated model variants resolve to their canonical catalog. Returns zero-value
// for unknown models.
func Lookup(model string) Pricing {
	p, _ := catalog.PrimaryPriceFor(model)
	return p
}
