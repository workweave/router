...
- **Azure BYOK endpoints are normalized onto `/openai/v1`.** An Azure OpenAI / Foundry resource serves the OpenAI wire format only under `/openai/v1`; a base URL stored the OpenAI way (bare host or `/v1`) answers 404 `Resource not found` on inference *and* on model discovery. `providers.NormalizeAzureOpenAIBaseURL` rewrites those two shapes and leaves every other path (deployment-scoped, project-scoped, gateway routes) untouched.
