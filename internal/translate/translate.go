...
func writeOpenAIUsageFromAnthropic(jw *jsonWriter, usage gjson.Result) {
	...
	jw.Obj()
	jw.Key("prompt_tokens"); jw.Int(input)
	jw.Key("completion_tokens"); jw.Int(output)
	jw.Key("total_tokens"); jw.Int(input + output)
	jw.EndObj()
}
...
func AnthropicToOpenAIError(body []byte) []byte {
	...
	jw.Obj()
	jw.Key("error"); jw.Obj()
	jw.Key("message"); jw.Str(errMsg)
	jw.Key("type"); jw.Str(errType)
...
func OpenAIToAnthropicResponse(body []byte, requestModel string) ([]byte, error) {
	...
	jw.Key("stop_reason"); jw.Str(openAIFinishToAnthropicStopReason(finishReason))
...
func writeAnthropicUsageFromOpenAI(jw *jsonWriter, usage gjson.Result) {
	...
	jw.Obj()
	jw.Key("input_tokens"); jw.Int(freshInput)
	jw.Key("output_tokens"); jw.Int(completion)
	if cacheRead > 0 {
		jw.Key("cache_read_input_tokens"); jw.Int(cacheRead)
	}
	if cacheCreation > 0 {
		jw.Key("cache_creation_input_tokens"); jw.Int(cacheCreation)
	}
	jw.EndObj()
}
