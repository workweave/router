...
func TestDecide_ObservedInputTokensFloorsEstimate(t *testing.T) {
	...
	assert.Equal(t, planner.OutcomeSwitch, switched.Outcome, "observed 50k floor must flip to switch")
	assert.Equal(t, planner.ReasonEVPositive, switched.Reason)
	...
	got := planner.Decide(estimateWins, defaultCfg)
	assert.Equal(t, planner.OutcomeSwitch, got.Outcome, "larger estimate must win the max()")
	assert.InDelta(t, 0.063, got.ExpectedSavingsUSD, 1e-9)
	assert.InDelta(t, 0.036, got.EvictionCostUSD, 1e-9)
}
