// Package hmm delegates model selection to the HMM router sidecar.
//
// The sidecar owns HMM/classifier/bandit inference and returns a target model
// plus display metadata. The router still builds the eligible candidate set
// from its catalog and dispatches through its normal provider machinery.
package hmm
...
func reasonFor(res Result) string {
	...
	if res.ComplexityLabel != "" {
		return "hmm_policy(label=" + res.ComplexityLabel + ")"
	}
	return "hmm_policy"
}
