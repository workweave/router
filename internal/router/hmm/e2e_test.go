...
func TestHTTPRouterEndToEndWithSidecar(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/route":
			...
			_ = json.NewEncoder(w).Encode(routeResponse{
				...
				Reason:          "classifier 'SPAWN_EXPLORE'",
				Cluster:         "explore",
				ComplexityLabel: "SPAWN_EXPLORE",
				Propensity:      0.5,
				DisplayMarker:   "✦ **Weave Router** → Spawning agent with moonshotai/kimi-k2.7-code\n↳ label: SPAWN_EXPLORE",
			})
		case "/outcome":
	...
	decision, err := r.Route(context.Background(), router.Request{
		PromptText:           "please explore the HMM router source",
	...
	assert.Contains(t, decision.Metadata.DisplayMarker, "Spawning agent")
	assert.Equal(t, "please explore the HMM router source", gotRoute.PromptText)
