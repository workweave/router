...
- **Don't add fail-open fallbacks _in this package_.** The scorer's contract is "score honestly or fail honestly": it returns `ErrClusterUnavailable` on every failure path (embed timeout, embed error, dim mismatch, prompt too short, empty argmax) and never substitutes a model. This is load-bearing for eval/regression detection. The previous in-`heuristic` fallback was removed because it silently degraded routing — every request that should have hit the cluster scorer instead got `claude-haiku-4-5`, masking real regressions in eval + prod. New failure modes return the sentinel; no default-model shortcut "for safety". **Degradation policy is the orchestrator's job, not the scorer's.** `proxy.Service`, when `ROUTER_SCORER_FALLBACK` is enabled, may serve the client's _explicitly requested_ model on `ErrClusterUnavailable` (see [`../../proxy/CLAUDE.md`](../../proxy/CLAUDE.md) → "Scorer-unavailable degradation"). That does **not** reintroduce the masking the `heuristic` removal fixed: it serves the caller's stated intent (not a fixed cheap default), it is _loud_ (ERROR log + `routing.degraded` span attr + `x-router-degraded` header + a visibly degraded user marker — strictly more observable than a 503 lost in client-retry noise), and it defaults **off** for `managed`/eval so the strict sentinel→503 path the eval harness depends on is unchanged. Keep this package fail-closed regardless of that knob.
