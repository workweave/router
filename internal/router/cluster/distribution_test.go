...
func TestRoutingDistribution_NoDeadZone_FlooredBundle(t *testing.T) {
	// Floored calibration keeps consecutive identical-mix runs short.
...
func TestRoutingDistribution_MidDialIsPricierThanLowDial_FlooredBundle(t *testing.T) {
	// The floored bundle uses a lower ratio threshold than the v0.67 bundle.
...
// loadV0_73 loads the committed v0.73 bundle (ships a non-empty AlphaFloor).
...
func TestComputeDialCalibration_NoBreakpointsBelowMinFloor(t *testing.T) {
	// No interior breakpoint should fall below min(AlphaFloor), where alphas
	// are unreachable.
	s := loadV0_73(t)
