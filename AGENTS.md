...
> [CLAUDE.md](CLAUDE.md). Different LLM tooling reads different filenames
> (Cursor and most generic agents read `AGENTS.md`; Claude Code reads
> `CLAUDE.md`), so we maintain both. **Update both files together** —
...
- Local-dev build environment (Apple Silicon dev box):
  ...
  releases at [https://github.com/daulet/tokenizers/releases/](https://github.com/daulet/tokenizers/releases/). Extract
...
- `model_registry.json` is a flat list of `{model, provider, bench_column, proxy?, proxy_note?}` entries. Direct columns are 1:1
...
- `cmd/router/main.go` wires `evalswitch.New(cluster_or_heuristic, heuristic)` so a single staging deployment can serve both routers
...
handler, and `providers.Client` grows a `Stream(ctx, req) (chan Event, error)` method or similar — design it then, not now.
