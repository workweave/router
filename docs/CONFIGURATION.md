...
| `ROUTER_RESPECT_REQUESTED_MODEL`  | *(none)*                     | Comma-separated models served verbatim when the client names one, bypassing planner and scorer — so a Claude Code skill or agent whose frontmatter pins a model keeps it. A list rather than a boolean because clients send the same `model` field on ordinary turns, so honoring every requested model would disable routing; list only the models you use as frontmatter pins. Aliases (`haiku`, `opus-4.8`) resolve to canonical IDs; unrecognized entries are dropped at boot. Automatic hard pins (probe, title-gen, compaction, sub-agent) still win, and an excluded or ineligible model falls back to automatic routing rather than failing the request. |
| `ROUTER_STATIC_CLUSTER_PIN`       | *(none)*                     | `clusterID:model` pairs, comma-separated (e.g. `0:claude-sonnet-5,1:claude-opus-5`). Pins every request whose nearest cluster (`top_p` rank 0) matches an entry to that model, skipping `blendScoresV2` entirely — an eval lever for measuring a pinning-only routing strategy against the cluster scorer's own latency overhead. A malformed pair or unresolvable cluster ID is logged and dropped at boot; a pinned model absent from the request's eligible set (excluded, wrong provider, image-incapable) falls through to normal cluster-scorer routing for that request rather than failing it. `Reason` is tagged `cluster-pin:` instead of `cluster:` so telemetry can separate the two paths without a schema change. Debugging/eval only. |
| `ROUTER_TOOL_RESULT_TIER_CEILING` | `false`                      | Excludes every model whose tier is above the turn's requested-model tier, applied only to `ToolResult` turns before the scorer runs. Unlike `Probe`/`TitleGen`/`Compaction`/`Classifier`, `ToolResult` carries no turn-type-specific routing constraint by default and can be upshifted by the scorer same as `MainLoop`; this caps that specifically, leaving `MainLoop` free to upshift when the scorer judges it worth the cost. Reuses `ExcludedModels`, so a sticky session pin already above the ceiling becomes ineligible and the turn re-routes rather than silently exceeding the cap. A requested model with an unknown tier is a no-op. Eval/debug lever. |
